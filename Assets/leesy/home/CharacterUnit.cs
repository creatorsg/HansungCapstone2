...
namespace Lsy
{
    ...
    public class CharacterUnit : NetworkBehaviour
    {
        ...
        /// <summary>FinalHeroPos 기준 초상화 슬롯 인덱스 (0~3). 설정되면 OnAnyUnitReady 발화.</summary>
        [SyncVar(hook = nameof(OnHeroPosChanged))]
        public int heroPos = -1;

        /// <summary>FinalHeroCode 기반 초상화 이미지 매핑용</summary>
        ...
        // --- UI 레이어용 이벤트 ---
        /// <summary>로컬 권한 획득 시점 초기화용</summary>
        public static event Action<CharacterUnit> OnLocalUnitSpawned;
        /// <summary>heroPos가 설정되었을 때 초상화 UI 갱신용 (전체 클라이언트)</summary>
        ...
        /// <summary>HP 또는 San이 변경되었을 때 초상화 슬라이더 갱신용</summary>
        ...
        /// <summary>
        /// 클라이언트에서 오브젝트가 완전히 초기화된 후 호출됩니다.
        /// 초기 스폰 시 SyncVar 훅이 발동하지 않는 Mirror 버전 대비용.
        /// heroPos가 이미 유효하다면 여기에서 명시적으로 OnAnyUnitReady를 발화합니다.
        /// </summary>
        ...
        /// <summary>
        /// CharacterSelect의 PlayerData 경로로 넘어온 데이터로 초기화합니다.
        /// 골드는 PlayerAccount에서 관리하므로 여기서는 설정하지 않습니다.
        /// </summary>
        [Server]
        public void SetupFromPlayerData(PlayerData pd)
        {
            ...
            heroPos = pd.FinalHeroPos; // hook 통해 OnAnyUnitReady 발화
            maxHp = pd.Info.Hp;
            maxSan = pd.Info.San;
            currentHp = maxHp;
            currentSan = maxSan;
        ...
        // 나중에 지울 것. 아래 AddItemWithInfo로 교체할 것
        [Server]
                ...
                if (myInventory[i].itemName == itemName)
                {
                    ...
                    switch (itemInInv.Type)
                    {
                        ...
                        case ItemType.Weapon:
                            ...
                            if (equipmentSlot.EquipWeapon(itemInInv))  // string 대신 InventoryItem
                            {
                                success = true;
                        ...
                        case ItemType.Armor:
                            ...
                            if (equipmentSlot.EquipArmor(itemInInv))  // string 대신 InventoryItem
                            {
                                success = true;
        ...
        public void CmdUnequipConsumableFromSlot(string itemName)
        {
            ...
            // 장착 슬롯에서 해당 아이템을 먼저 찾아 데이터를 복사해둡니다.
